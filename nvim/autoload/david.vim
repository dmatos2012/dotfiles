...
function! david#easy_autoload() abort
...
""
" List occurences
function! david#list_occurrences(...) abort
  if a:0 > 0
...
""
" Find project root
function! david#find_project_root(...) abort
...
function! david#dict_to_formatted_json(dict) abort
  ...
  call nvim_buf_set_lines(l:buffer_number, 0, -1, 1, std#json#format(david#json_encode(a:dict)))

  silent! call dictwatcherdel(a:dict, '*', 's:dict_watcher_func')

  function! s:dict_watcher_func(d, k, z) abort closure
    call nvim_buf_set_lines(l:buffer_number, 1, -1, 1, std#json#format(david#json_encode(a:d)))
  endfunction
...
function! david#profile( command, numberOfTimes )
...
function! david#code_review_yank() abort
...
function! david#visual_code_review() abort
...
vnoremap yc <c-r>:call david#visual_code_review()<CR>
xnoremap yc <c-r>:call david#viual_code_review()<CR>

if !exists('*david#save_and_exec')
  function! david#save_and_exec() abort
    if &filetype == 'vim'
      :silent! write
      :source %
    elseif &filetype == 'lua'
...
function! david#jump_direction(letter)
